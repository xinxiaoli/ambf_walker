...
class Read_Mocap(object):
    ...
    def get_next_point(self,dt):
        ...
        q[3 + offset] = 0#-math.radians(float(self.df["RHipAngles"][self.count]))
        q[4 + offset] = 0#math.radians(float(self.df["RKneeAngles"][self.count]))
        q[5 + offset] = 0#math.radians(float(self.df["RAbsAnkleAngle"][self.count]))

        if self.count == 0:
            qd = np.zeros(len(q))
        else:
            qd = 0# (self.q_last - q)/(dt+.0000000001)
